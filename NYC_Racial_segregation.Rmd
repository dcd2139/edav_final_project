...
To analyze how segregated are NYC schools among the zip codes, below datasets were joined with the demographic dataset, to source the zip codes for each school.
...
are distributed through out the Bronx.
...
For this we defined a metric called `segregation_score` to capture the level of segregation of each school in New York city. This allows us to compare different schools with different distribution of students' ethnicities. 
`segregation_score = prop_common - mean(prop_others) `
...
`segregation_score = (1.25 * prop_common) - 25`
...
- Schools where black students are the most common, the mean segregation score is much higher compared to other races. This could mean in schools where the proportion of Black students is the highest, the distribution of students is more skewed than schools where the proportion of Asian or White students is the highest.
- A similar pattern is also found in schools where hispanic students are the most common.
- We observe that for schools with Black, White or Asian students as the most common, the mean segregation score is decreasing over time. However, for schools with Hispanic students are the most common, the segregation score has remained more or less constant over time.
...
All ethnicities seems to be highly clustered in the map above which could be due to the underlying population being clustered along racial lines. `It is very surprising to see such contiguous clusters with the same race being the most common one`.
...
### Additional factors driving the racial segregation in New York city

We considered looking at the relationship between Poverty percentage and segregation score in NYC schools, as poverty is worth considering when talking about school diversity. We also looked at the relationship between economic need index and segregation score.
...
- The schools with the least poverty percentage do not have a very high rate of segregation. Only schools in which a greater percentage of students are in poverty, have very high segregation.
- Also, in schools where the average poverty percentage is low, the most common ethnicity of students is overwhelmingly white. There are also very few schools with a high proportion of Black students and a low poverty percent. This could be a reflection on differences in the distribution of wealth among people of different races.
...
- All ethnicities seems to be highly clustered in the Choropleth map above for NYC school enrollments, which could be due to the underlying population being clustered along racial lines. It is very surprising to see such contiguous clusters with the same race being the most common one.
